...
CI (the secret_scan smoke + the SKILL.md frontmatter check) never scanned for
client/engagement identifiers at all — and a client name once reached this repo's
tree. This scanner closes that gap: it hashes every 1- and 2-word shingle of EVERY
tracked text file and compares against scripts/.identifier-denylist.sha256 (plus an
optional gitignored scripts/.identifier-denylist.local). Plaintext names never live
in the repo. (Ported from Claude-BugHunter's scan_identifiers.py — same mechanism.)
